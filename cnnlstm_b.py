...
LEARNING_RATE_DECAY_FACTOR = 0.99#0.98 # The learning rate decay factor
...
#----------------------------------------__>
                   ...
                   x = layers.BatchNormalization()(x)
                   x = max_pool(x, ksize=(2, 2), stride=(2, 1))                   
                   #print (x.get_shape())                    
                   #---------------------__>
...
#                   x = layers.BatchNormalization()(x)
                   ...
                   #x = layers.Conv2D(128, (1, 2), activation='relu', padding="valid")(x)
                   #x = layers.Conv2D(128, (1, 2), activation='relu', padding="valid")(x)
                   #x = layers.Conv2D(64, (1, 2), activation='relu', padding="valid")(x)
                   #x = layers.BatchNormalization()(x)
                   print (x.get_shape())
                   h_pool5 = x#max_pool(x, ksize=(2, 2), stride=(2, 1))
...
#                    tf.squeeze(input_tensor, axis=1)  
                    ...
                    W_fc1 = weight_variable([1 * 4 * 64, out_channels])
                    b_fc1 = bias_variable([out_channels])
                    conv_layer_flat = tf.reshape(h_pool5, [-1, 1 * 4 * 64])
                    print (conv_layer_flat.get_shape())
                    features = tf.nn.relu(tf.matmul(conv_layer_flat, W_fc1) + b_fc1)
                    print (features.get_shape())
                    shape = tf.shape(features)
                    x = tf.reshape(features, [shape[0], out_channels, 1])   #batchsize * outputshape * 1
                    print (x.get_shape())
#---------------------------------------->                   
...
class DataIterator:
    ...
    def input_index_generate_batch(self, index=None):
        ...
        def get_input_lens(sequences): 
            ...
            lengths = np.asarray([out_channels for _ in sequences], dtype=np.int64) # + ADD SEQ
...
def train():
    ...
    def do_report():#Информация/сохранение модели
        indexs_test = [shuffle_idx_val[i % num_val_samples] for i in range(batch * B_SIZE, (batch + B_SIZE) * B_SIZE)]    
        test_inputs, test_seq_len, test_labels = val_feeder.input_index_generate_batch(indexs_test)
        test_feed = {m.inputs: test_inputs, m.labels: test_labels, m.seq_len: test_seq_len}
        #test_feed = {m.inputs: val_inputs, m.labels: val_labels}
        dd, log_probs, accuracy = session.run([decoded[0], log_prob, acc], test_feed)
        accuracy = report_accuracy(dd, test_labels)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, log_device_placement=True)) as session:
		...
		train_inputs, train_seq_len, train_labels = train_feeder.input_index_generate_batch(indexs)#utils.get_data_set('LSTM2', batch * common.BATCH_SIZE, (batch + 1) * common.BATCH_SIZE)
            ...
            avg_train_cost = train_cost / ((cur_batch + 1) * B_SIZE)
            #train_cost /= B_SIZE
            indexs_val = [shuffle_idx_val[i % num_val_samples] for i in range(cur_batch * B_SIZE, (cur_batch + B_SIZE) * B_SIZE)]    
            ...
            log = "Epoch {}/{}, steps = {}, avg_train_cost = {:.3f}, val_cost = {:.3f}, val_ler = {:.3f}, time = {:.3f}s, learning_rate = {}"
            print(log.format(curr_epoch + 1, num_epochs, steps, avg_train_cost, val_cost, val_ler, time.time() - start, lr))
